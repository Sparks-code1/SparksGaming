...
interface Props {
  ...
  onRuin: (territoryId: string) => void
  onDecline: () => void
}

/**
 * Die Humans event resolution — the Alien player may replace a minor city
 * with a Ruin. Ruining removes all troops, demolishes any HQ, destroys any
 * fortification, and destroys the event card forever. Declining returns the
 * card to the event discard pile.
 */
export default function DieHumansModal({ legacyState, territories, alienPlayerName, onRuin, onDecline }: Props) {
  ...
  const candidates = legacyState.stickers
    .filter(s =>
    ...
    .map(s => ({ sticker: s, territory: territories[s.targetId] }))
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'center', marginBottom: 18 }}>
          ...
          <div style={{ fontSize: 12, color: '#7a9a8a', marginTop: 6, lineHeight: 1.5 }}>
            <strong style={{ color: '#00c8a0' }}>{alienPlayerName}</strong> may replace a minor city with a Ruin.
        ...
        <div style={{
          ...
          Minor cities on the board
        </div>
        <div style={{
          display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 20,
          maxHeight: 200, overflowY: 'auto',
        }}>
          {candidates.map(({ territory }) => {
            const sel = selected === territory!.id
            return (
              <button
                key={territory!.id}
                onClick={() => setSelected(sel ? null : territory!.id)}
                style={{
        ...
        {selected && (
          <div style={{
            ...
            fontSize: 11, color: '#c07a6a', lineHeight: 1.5,
          }}>
            ⚠ {territories[selected]?.name} will be reduced to a Ruin — troops removed, city gone,
            HQ demolished, fortification destroyed. This card will be <strong>destroyed forever</strong>.
          </div>
        )}
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          ...
          >
            Decline — return card to deck
          </button>
          <button
            disabled={!selected}
            onClick={() => selected && onRuin(selected)}
            style={{
