...
export const TREACHERY_CARDS: TreacheryCard[] = [
  ...
  },
  {
    ...
    text: 'Play during the Mentat Pause. You control the NEXT turn\'s storm and may move it'
      + ' from 0 to 10 sectors in a counterclockwise direction.'
      + '\n\nTHIS GAME: at the Pause, not during the Storm Phase — which is the moment just'
      + ' before, so the sequence is unchanged.',
  },
  {
    id: 'karama',
    name: 'Karama',
  ...
  },
  {
    ...
    text: 'Play during the Mentat Pause, only if you have forces on the Shield Wall or a territory'
      + ' adjacent to it with no storm between your sector and the Wall.'
      + '\n\nAll forces on the Shield Wall are destroyed. The Imperial Basin, Arrakeen and Carthag'
      + ' are no longer protected from the storm, from next turn\'s on.'
      + '\n\nTHIS GAME: at the Pause, not after the storm is calculated — so the coming roll is not'
      + ' yet known.',
  },
]
