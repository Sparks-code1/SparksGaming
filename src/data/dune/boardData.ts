...
export interface DuneTerritory {
  id: string
  displayName: string
  /** The outline the board prints, verbatim from the artwork's own path.
   *  Copied rather than referenced because the printed board is injected as
   *  separate markup: an overlay cannot reliably restyle a shape inside it,
   *  and a second set of coordinates traced by hand would be a second set to
   *  drift. This one comes off the same path in the same build. */
  outline: string
...
export interface DuneCell {
  sector: string
  at: { x: number; y: number }
  areaShare: number
  /** Distance from 'at' to the territory's edge: the largest disc that
   *  fits here. What the board fans several factions' bubbles inside. */
  room: number
}
