...
function fixedPlacement(faction) {
...
function openingPosition(input) {
  ...
  const forces = seats.map((s) => fixedPlacement(s.faction)).filter((f) => f !== null);
...
function landPlacement(forces, placed) {
  let out = forces;
  for (const p of placed) {
    out = p.posture === "advisor" ? [...out, p] : landForces(
      out,
      p.faction,
      p.territoryId,
      p.sector,
      p.count,
      Math.min(p.count, p.starred ?? 0)
    );
  }
  return [...out];
}
function shipAdvisor(players, faction, placed) {
  const sent = placed.filter((f) => f.faction === faction).reduce((n, f) => n + f.count, 0);
  if (sent <= 0) return [...players];
  return players.map((p) => p.faction === faction ? { ...p, reserves: Math.max(0, p.reserves - sent) } : p);
}
